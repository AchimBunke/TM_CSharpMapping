...
internal class SocketDefV3
{
    public Matrix4x4 GlobalTransform { get; set; }
    public CPlugSpawnModel? WaypointSpawnModel { get; set; }
}

internal class FbxMeshConverterV3
{
    public static ToolResult<List<NodeDefV3>> ExtractMeshNodes(ImportedScene scene, FbxGbxConversionInput config)
    {
        List<NodeDefV3> nodeDefs = [];
        ...
        foreach (var node in nodes)
        {
            ...
            nodeDefs.Add(new NodeDefV3 { Node = node, NodeConfig = meshConfig, GlobalTransform = node.GlobalTransform });
        }
        return ToolResult.Success(nodeDefs, nameof(FbxGbxConverterV3));
    }

    public static ToolResult<List<SocketDefV3>> ExtractSockets(ImportedScene scene, FbxGbxConversionInput config)
    {
        List<SocketDefV3> socketDefs = new List<SocketDefV3>();
    ...
    /// <summary>
    /// Groups mesh nodes into <see cref="NodeDefGroupV3"/> buckets (static/dyna/trigger/waypoint, LOD-split)
    ...
    public static ToolResult<List<NodeDefGroupV3>> GroupNodes(List<NodeDefV3> nodes, List<SocketDefV3> sockets, FbxGbxConversionInput config)
    {
        ...
        var grouper = new NodeGrouperV3(lods, config.ItemConfig);
        ...
        if (sockets.Count > 0)
        {
            ...
            {
                if (group.Type != ModelTypeV3.Trigger_Waypoint)
                    continue;
                if (group.WaypointType == GBX.NET.Engines.GameData.CGameItemModel.EWaypointType.Finish)
                    continue;
                group.WaypointSpawnModel = new CPlugSpawnModelHolderV3 { SpawnModel = sockets[0].WaypointSpawnModel! };
            }
        }

        return ToolResult.Success(groups, nameof(FbxGbxConverterV3));
    }
    ...
    /// </summary>
    public static (System.Numerics.Vector3 Position, System.Numerics.Quaternion Rotation) ComputeGroupAnchor(NodeDefGroupV3 group)
    {
    ...
    public static ToolResult<None> ExtractMeshes(
        ImportedScene scene,
        List<NodeDefGroupV3> groups,
        List<MaterialDef> materials,
        List<NodeDefV3> nodes,
        ...
        Dictionary<int, System.Numerics.Quaternion> groupIndexToAnchorRotation)
    {
        ...
        return ToolResult.Success(None.Value, nameof(FbxGbxConverterV3));
    }

    static MeshPropertiesV3 ComputeMeshProperties(MeshConfig meshConfig)
    {
        var properties = MeshPropertiesV3.Enabled;
        ...
        if (meshConfig.Lods.Count > 0)
            properties |= MeshPropertiesV3.LOD;
        return properties;
    }
    ...
    internal static ToolResult<MeshConfig> FindMeshConfigForMesh(string meshName, FbxGbxConversionInput config)
    {
        ...
        if (meshConfig is null && !configFromMeshName)
            return ToolResult.Fail(nameof(FbxGbxConverterV3), ErrorCodes.FbxGbxConverter.MissingMeshConfig, meshName);
        ...
        return ToolResult.Success(meshConfig!, nameof(FbxGbxConverterV3));
    }
    ...
    private static NormalizedMeshV3 MergeMeshes(IEnumerable<NormalizedMeshV3> meshes)
    {
        var list = meshes.ToList();

        var result = new NormalizedMeshV3
        {
    ...
    private static void DeduplicateVertices(NormalizedMeshV3 mesh)
    {
        var vertexMap = new Dictionary<VertexKeyV3, int>();
    ...
    private readonly record struct VertexKeyV3(
        Vec3 Position,
        Vec3 Normal,
        Vec2? TexCoord,
        Vec2? LightmapCoord,
