...
/// <summary>
/// Converts FBX scenes into <see cref="NormalizedItemV3"/>/<see cref="CGameItemModel"/> using the new
...
/// NormalizedItemV3 natively supports a position/rotation hierarchy.
/// </summary>
public class FbxGbxConverterV3
{
    public ToolResult<CGameItemModel> ConvertToGbx(FbxGbxConversionInput conversionInput, CompileOptions? compileOptions = null)
    {
        ...
        return ToolResult.Success(itemResult.Value, nameof(FbxGbxConverterV3));
    }
    ...
    public ToolResult<CGameItemModel> ConvertToGbxAndSaveItem(FbxGbxConversionInput conversionInput, CompileOptions? compileOptions = null)
    {
        if (string.IsNullOrWhiteSpace(conversionInput.ItemOutputPath))
            return ToolResult.Fail(nameof(FbxGbxConverterV3), ErrorCodes.FbxGbxConverter.InvalidItemOutputPath);
    ...
    public ToolResult<NormalizedItemV3> ConvertToNormalizedItem(FbxGbxConversionInput conversionInput)
    {
        ImportedScene scene;
        try
        {
        ...
        {
            return ToolResult.Fail<NormalizedItemV3>(nameof(FbxGbxConverterV3), ErrorCodes.FbxGbxConverter.FbxParsingError, e);
        }
    ...
    {
        var item = new NormalizedItemV3();

        var materialConverter = new FbxMaterialConverterV3(config.MaterialLibrary);
        ...
        var nodeResults = FbxMeshConverterV3.ExtractMeshNodes(scene, config);
        if (nodeResults.IsFailure)
            return ToolResult.Fail(nodeResults);

        var socketResults = FbxMeshConverterV3.ExtractSockets(scene, config);
        if (socketResults.IsFailure)
            return ToolResult.Fail(socketResults);
        if (socketResults.Value.Count > 1)
            return ToolResult.Fail(nameof(FbxGbxConverterV3), ErrorCodes.FbxGbxConverter.MultipleSocketsNotSupported);

        var lightResults = FbxLightConverterV3.ExtractLights(scene, config);
        ...
        var rootModel = new NormalizedModelV3 { Type = ModelTypeV3.Container };
        int rootKey = 0;
        ...
        for (int i = 0; i < groups.Count; i++)
        {
            ...
            var (anchorPos, anchorRot) = FbxMeshConverterV3.ComputeGroupAnchor(group);
        ...
        for (int i = 0; i < groups.Count; i++)
        {
            ...
            if (!string.IsNullOrEmpty(group.RelativeMovingParentGroupId))
            {
                var parentIndex = groups
                    .Select((g, idx) => (g, idx))
                    .Where(t => t.g.Type == ModelTypeV3.Dynamic)
        ...
        var meshResult = FbxMeshConverterV3.ExtractMeshes(scene, groups, materialResults.Value, nodes, config, item,
        ...
        return ToolResult.Success(item, nameof(FbxGbxConverterV3));
    }

    void AssignLightsToGroups(
        NormalizedItemV3 item,
        List<LightDefV3> lights,
    ...
    {
        int firstStaticGroupIndex = groups.FindIndex(g => g.Type == ModelTypeV3.Static);
    ...
    void SetMetaData(NormalizedItemV3 item, FbxGbxConversionInput config)
    {
    ...
    List<NodeDefV3> FilterAndApplySpecialMeshItems(ImportedScene scene, IEnumerable<NodeDefV3> nodes, List<MaterialDef> materials)
    {
        List<NodeDefV3> nodesWithMesh = new List<NodeDefV3>();
