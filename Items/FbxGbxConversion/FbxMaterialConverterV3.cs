...
/// <summary>
/// V3 counterpart of <see cref="FbxMaterialConverter"/>. Consumes the importer-neutral
...
internal class FbxMaterialConverterV3
{
    ...
    public ToolResult<List<MaterialDef>> ExtractMaterials(ImportedScene scene, FbxGbxConversionInput config)
    {
        ...
        return ToolResult.Success(customMaterial, nameof(FbxGbxConverterV3));
    }
    ...
    ToolResult<MaterialDef> ConvertMaterial(ImportedMaterial mat, FbxGbxConversionInput config)
    {
        ...
        if (!TryFindMaterialConfig(matName, config, out var materialConfig))
            return ToolResult.Fail(nameof(FbxGbxConverterV3), ErrorCodes.FbxGbxConverter.MissingMaterialConfig, matName);
        ...
        return ToolResult.Success(materialDef, nameof(FbxGbxConverterV3));
    }
